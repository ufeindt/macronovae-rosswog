...
WHOLESKY_SQDEG = sncosmo.simulation.WHOLESKY_SQDEG
_cosmo = Planck15
...
class RateCalculator(object):
    ...
    def __init__(self, model, band='bessellb',
...
def shell_rate(zmin, zmax, ratefunc=(lambda z: 3e-7),
               nbins=100, cosmo=_cosmo, time=365.25, area=WHOLESKY_SQDEG):
    """
    rate in Mpc^-3 yr^-1
    
    returns shell rate and bin centers
    """
    f = time / 365.25 * area / WHOLESKY_SQDEG
